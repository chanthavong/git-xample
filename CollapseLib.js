var CollapseLib = {
  ...
  // https://gist.github.com/alabid/3154631

  // if you have an empty object, just return an empty object
  collapseDict: function(complexDict, fullKey = false) {
      ...
      for (var complexKey in complexDict) {
        ...
        if (CollapseLib.isPlainObject(complexDict[complexKey])) {
          ...
          for (var subKey in subDict) {
            if (fullKey) {
              plainDict[complexKey + '.' + subKey] = CollapseLib.collapseDict(
                subDict[subKey]
              )
            } else {
              plainDict[subKey] = CollapseLib.collapseDict(subDict[subKey])
            }
          }
        } else if (Array.isArray && Array.isArray(complexDict[complexKey])) {
          if (!CollapseLib.isComplexArray(complexDict[complexKey])) {
            plainDict[complexKey] = CollapseLib.getStrFromArray(
              complexDict[complexKey]
            )
          } else {
            sawComplex = true
            for (var i = 0; i < complexDict[complexKey].length; i++) {
              plainDict[complexKey + '[' + i + ']'] = CollapseLib.collapseDict(
                complexDict[complexKey][i]
              )
            }
          }
        } else {
          plainDict[complexKey] = CollapseLib.collapseDict(
            complexDict[complexKey]
          )
        }
      }
  ...
  isComplexArray: function(arr) {
    for (var i = 0; i < arr.length; i++) {
      if (
        (Array.isArray && Array.isArray(arr[i])) ||
